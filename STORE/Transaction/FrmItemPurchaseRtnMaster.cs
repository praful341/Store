...
{
    public partial class FrmItemPurchaseRtnMaster : Form
    {
        ...
        public FrmItemPurchaseRtnMaster(string Type_)
        {
            InitializeComponent();
            Form_Type = Type_;
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow DRow in DTab.Rows)
            {
                ...
                Invoice_EntryProperty.IGST_Amt = 0;
                Int64 ItemPurchaseRetMasterID = Val.ToInt64(DRow["ItemPurchaseRtnMasterID"]);
                if (ItemPurchaseRetMasterID == 0)
                {
                    Invoice_EntryProperty.ItemPurchaseRtnMasterID = Val.ToInt64(Newmstid);
                    Invoice_EntryProperty.ItemPurchaseReturnDtlID = Val.ToInt64(DRow["ItemPurchaseReturnDtlID"]);
                }
                else
                {
                    Invoice_EntryProperty.ItemPurchaseRtnMasterID = Val.ToInt64(DRow["ItemPurchaseRtnMasterID"]);
                    Invoice_EntryProperty.ItemPurchaseReturnDtlID = Val.ToInt64(DRow["ItemPurchaseReturnDtlID"]);
                }
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            //txtInvoiceNo.Text = ObjInvoiceEntry.FindNewInvoiceNo(Form_Type).ToString();
            //txtInvoiceNo.Enabled = false;

            string Invoice_No = Val.ToString(ObjInvoiceEntry.GEtMaximumID_New("Purchase_Return"));
            txtInvoiceNo.Text = Invoice_No.ToString(); //ObjInvoiceEntry.FindNewInvoiceNo(Form_Type).ToString();
        ...
        private void gridView2_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                if (e.Clicks == 2)
                {
                    ...
                    txtPurchaseMasterID.Text = Val.ToString(DRow["ItemPurchaseRtnMasterID"]);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            FrmReportViewer.ShowForm("Purchase_Memo_Sale", 120, Report.FrmReportViewer.ReportFolder.ACCOUNT);
        ...
        private void GrdPurchase_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F9)
            {
                if (Global.Confirm("Are you sure delete selected row?", "STORE", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                {
                    Invoice_EntryProperty Invoice_EntryProperty = new Invoice_EntryProperty();
                    int IntRes = 0;
                    Int64 ItemPurchaseMasterID = Val.ToInt64(dgvPurchase.GetFocusedRowCellValue("ItemPurchaseRtnMasterID").ToString());
                    Invoice_EntryProperty.ItemPurchaseMasterID = Val.ToInt64(ItemPurchaseMasterID);
                    Invoice_EntryProperty.ItemPurchaseDetailID = Val.ToInt64(dgvPurchase.GetFocusedRowCellValue("ItemPurchaseReturnDtlID").ToString());
